...
def applyFF(data, sampleFreq):
    if sampleFreq > 3:
        sos = sig.iirdesign([0.66, 3.0], [0.5, 4.0], 1.0, 40.0, fs=sampleFreq, output="sos")
...
def readIntensity(intensities, curFrame, cropBoxBounds):
    ...
    while True:
        ...
        if face is not None:
            ...
            cv2.imshow("ROI", intensity)
            # intensity = np.median(ROI) # works, but quite chunky.

            intensities.append(intensity)

            # Draw the forehead box:
            curFrame[0] = cv2.rectangle(frame, (eyeleft, headTop), (eyeright, eyeTop), (0, 255, 0), 1)
