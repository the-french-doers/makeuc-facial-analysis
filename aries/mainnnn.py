...
realWidth = webcam.get(cv2.CAP_PROP_FRAME_WIDTH)
realHeight = webcam.get(cv2.CAP_PROP_FRAME_HEIGHT)
videoChannels = 3
videoFrameRate = 15
...
while True:
    ...
    cv2.rectangle(
        frame,
        (int(videoWidth / 2), int(videoHeight / 2)),
        (int(realWidth - videoWidth / 2), int(realHeight - videoHeight / 2)),
        boxColor,
        boxWeight,
    )
    cv2.putText(
        frame, "Place you face inside the Rectangle", loadingTextLocation, font, fontScale, (255, 0, 0), lineType
    )
    if i > bpmBufferSize:
